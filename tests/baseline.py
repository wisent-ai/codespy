...
Usage:
    python3 tests/baseline.py [--dry-run]
"""
...
def resolve(root: pathlib.Path) -> tuple:
    ...
    if metadata is not None:
        ...
        return marker, version, names, (
            ...
            "tests/surface.py. This is the artifact users install, so no weaker tier "
            "applies."
        )
    ...
    if tagged is not None:
        ...
        return marker, version, names, (
            f"reproduced with `git archive {tag}` and read by tests/surface.py. PyPI "
